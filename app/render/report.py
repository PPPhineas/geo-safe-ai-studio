...
def render_report(placeholders: dict, judgement: dict, figures: dict[str, str]) -> str:
    ...
    repl = {
        ...
        "deformation_trend_summary": placeholders.get("deformation_trend_summary", "（未生成）"),
        "deformation_points_detail": placeholders.get("deformation_points_detail", "（无）"),
        "time_series_summary": placeholders.get("time_series_summary", "未接入有效传感器时序数据"),
        "time_series_points_detail": placeholders.get("time_series_points_detail", "（无有效时序趋势点位）"),
        "trend_forecast_summary": placeholders.get("trend_forecast_summary", "未接入有效传感器时序数据，暂不生成趋势预测。"),
        "trend_forecast_detail": placeholders.get("trend_forecast_detail", "（无趋势预测点位）"),
        "rain_deformation_summary": placeholders.get("rain_deformation_summary", "未开展降雨-变形耦合分析。"),
        "rain_deformation_detail": placeholders.get("rain_deformation_detail", "（无）"),
        "warning_history_summary": placeholders.get("warning_history_summary", "未接入有效预警历史。"),
        "warning_history_detail": placeholders.get("warning_history_detail", "（无）"),
